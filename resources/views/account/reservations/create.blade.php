...
@section('title', __('app.book_table'))

@section('content')
    <x-card :title="__('app.choose_slot')">
        <form method="GET" action="{{ route('account.reservations.create') }}" class="toolbar">
            <div class="field">
    ...
    <form method="POST" action="{{ route('account.reservations.store') }}">
                ...
                <div class="floor">
                    @foreach ($tables as $table)
                        <label class="floor__table is-free">
                            ...
                            <span class="small price">@money($table->price_per_hour) / {{ __('app.hour') }}</span>
                        </label>
                    @endforeach
                </div>
            @endif
        ...
        <x-card :title="__('app.reservation_details')" style="margin-top:var(--space-3)">
            <x-field name="special_requests" :label="__('app.special_requests')" :hint="__('app.special_requests_hint')">
                <x-textarea name="special_requests" rows="3" />
            </x-field>
            <button class="btn btn--block" type="submit" @disabled($tables->isEmpty())><x-icon name="calendar" />{{ __('app.confirm_booking') }}</button>
        </x-card>
    </form>
@endsection
