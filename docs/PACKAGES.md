...
| Redis | 6 or later | Cache and queue driver |
...
| `predis/predis` | Redis client for the cache and queue connections, chosen over the `phpredis` extension so no compilation step is needed |
...
### Packages deliberately removed from the original project

| Package | Reason |
| --- | --- |
| `laravel/sanctum` | The application is web only and uses the session guard. Token authentication has no role here. |
| OTP package | Email verification now stores a six digit code in the cache with an explicit expiry, which removes a dependency and keeps the logic visible in `AuthService`. |
...
# Queue worker; all three named queues plus the default queue
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `Connection refused` on any page | Redis is not running while `CACHE_DRIVER=redis` | Start Redis, or set `CACHE_DRIVER=file` and `QUEUE_CONNECTION=sync` for local work |
| Meal photos return 404 | The storage symlink is missing | `php artisan storage:link` |
| Mail is never delivered | The queue worker is not running | Start `queue:work`, or set `QUEUE_CONNECTION=sync` |
